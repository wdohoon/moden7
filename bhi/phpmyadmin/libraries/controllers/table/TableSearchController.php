...
class TableSearchController extends TableController
{
    ...
    public function zoomSubmitAction($dataLabel, $goto)
    {
        ...
        while ($row = $this->dbi->fetchAssoc($result)) {
            ...
            $row['where_clause_sign'] = Util::signSqlQuery($uniqueCondition[0]);
            ...
            $tmpData = array(
                ...
                'where_clause_sign' => Util::signSqlQuery($uniqueCondition[0])
            );
    ...
    {
        if (! Util::checkSqlQuerySignature($_REQUEST['where_clause'], $_REQUEST['where_clause_sign'])) {
            return;
        }
