...
$qry = "select * from mine_p_log where rno = '$no' ORDER BY `rdate` DESC;";
$DB_LP->select($qry);
...
while( $row = $DB_LP->get_data() )
{
	...
	if ( $row->type == 1 )
		$data[$idx]['type'] = "<font color = 'red'>믿음 리워드</font>";
	else if ( $row->type == 2 )
		$data[$idx]['type'] = "소망 리워드";
	else if ( $row->type == 3 )
		$data[$idx]['type'] = "사랑 리워드";

	$data[$idx]['coin'] = $row->coin;
	$data[$idx]['amount'] = $row->amount;
	$data[$idx]['amountp'] = $row->amount *	$OKNAP;
	$idx++;
}
				...
				<li>
					<div class="box">
						<dl>
							...
							<dd><?php echo  number_format( $data[$i]['amount'], 4 );?> OKNA</dd>
						</dl>
						<dl class="date">
