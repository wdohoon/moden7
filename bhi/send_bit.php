...
function sendTrc20Token($fromAddress, $toAddress, $amount, $token, $privateKey) {
    ...
    if ($err) {
        ...
        $data = json_decode($response, true);
        echo "<pre>sendTrc20Token response: " . json_encode($data, JSON_PRETTY_PRINT) . "</pre>";
        if (isset($data['data']['txid'])) {
            return $data['data']['txid'];
        } else {
            echo "<pre>sendTrc20Token failed: " . json_encode($data, JSON_PRETTY_PRINT) . "</pre>";
            return false;
        }
    }
}
            ...
            <form method="post" action="send.php" name="form">
                <div class="mb15">
                    <select class="select" id="coinSelect" name="coin">
                        <option value="btc" data-price="<?php echo $btcPrice; ?>" data-balance="<?php echo $BTC; ?>" data-gas="0.0005">비트코인</option>
                    </select>
                    <p id="priceDisplay"></p>
                </div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function updatePriceDisplay() {
        ...
        if (price > 0) {
            ...
            priceDisplay.innerHTML = `${coinName} 시세 정보를 가져올 수 없습니다.`;
        }
    }
    ...
    function calculateGPoint() {
        ...
        const gasFee = parseFloat(selectedOption.getAttribute("data-gas"));
        return (!isNaN(price) && !isNaN(amount) && !isNaN(gasFee)) ? ((amount - gasFee) * price) : 0;
    }
