...
include_once(G5_PATH.'/vendor/autoload.php'); // BlockSDK를 사용하기 위한 autoload 포함
...
if($w=='u'){
    ...
    $sql_certify .= " , mb_sex = '".$_POST['mb_sex']."' ";

    $old_pass = $_POST['old_pass'];

    if(!login_password_check($member, $old_pass, $member['mb_password'])){
        alert('기존 비밀번호가 다릅니다.');
    }
    ...
    $sql_certify .= " , mb_birth = '".$mb_4.'-'.$mb_5.'-'.$mb_6."' ";
    $sql_certify .= " , mb_sex = '".$_POST['mb_sex']."' ";
}
...
if ($w == '') {
	...
	// Bitcoin 지갑 생성
	$btcWallet = createBitcoinWallet("moden7_4307725", "5u4a1dq58g0gogs8g4sgo0800sskc400wcgk0osck84cs0swswc084k88o04808k");

	// Ethereum 지갑 생성
	$ethWallet = createEthereumWallet($blockSDK);

	// Tron 지갑 생성
	$tronWallet = createTronWallet("5u4a1dq58g0gogs8g4sgo0800sskc400wcgk0osck84cs0swswc084k88o04808k");

	// BSC 지갑 생성
	$bscWallet = createEthereumWallet($blockSDK);

	// 회원 테이블에서 no 값 가져오기
	$result = sql_fetch("SELECT mb_no FROM {$g5['member_table']} WHERE mb_id = '$mb_id'");
	$no = $result['mb_no'];

	// 프라이빗 키 암호화
	$btcPrivateKeyEncrypted = encrypt_private_key($btcWallet['privateKey']);
	$ethPrivateKeyEncrypted = encrypt_private_key($ethWallet['privateKey']);
	$tronPrivateKeyEncrypted = encrypt_private_key($tronWallet['privateKey']);
	$bscPrivateKeyEncrypted = encrypt_private_key($bscWallet['privateKey']);

	// 지갑 정보를 wallet_address 테이블에 저장
	$sql = "INSERT INTO wallet_address (no, mb_id, bit, bit_privary, eth, eth_privary, trx, trx_privary, pin)
			VALUES ('$no', '$mb_id', '{$btcWallet['address']}', '$btcPrivateKeyEncrypted', '{$ethWallet['address']}', '$ethPrivateKeyEncrypted', 
			'{$tronWallet['address']}', '$tronPrivateKeyEncrypted', '{$mb_1}')";
	sql_query($sql);
...
function createBitcoinWallet($walletId, $apiKey) {
    // Step 1: 지갑 주소 생성
    $curl = curl_init();
...
function createEthereumWallet($blockSDK) {
    $ethClient = $blockSDK->createEthereum();
    $address = $ethClient->createAddress(["name" => "test"]);
    return [
...
function createTronWallet($api_key) {
    ...
    if ($httpcode == 200) {
        ...
        curl_close($ch);
        return [
            'address' => $wallet_info['data']['address'],
            'privateKey' => $wallet_info['data']['privateKey']
        ];
    } else {
        curl_close($ch);
