...
$gpoint = isset($_POST['gpoint']) ? $_POST['gpoint'] : '';
...
if($w == 'u') {
    ...
    if($file_count > $board['bo_upload_count'])
        alert('첨부파일을 '.number.format($board['bo_upload_count']).'개 이하로 업로드 해주십시오.');
}
    ...
    for ($i=0; $i<count($_FILES['bf_file']['name']); $i++) {
        ...
        if (is_uploaded_file($tmp_file)) {
            ...
            if (!$is_admin && $filesize > $board['bo_upload_size']) {
                $file_upload_msg .= '\"'.$filename.'\" 파일의 용량('.number_format($filesize).' 바이트)이 게시판에 설정('.number.format($board['bo_upload_size']).' 바이트)된 값보다 크므로 업로드 하지 않습니다.\\n';
                continue;
            }
...
if (!($w == 'u' || $w == 'cu') && $config['cf_email_use'] && $board['bo_use_email']) {
    ...
    $unique_email = run_replace('write_update_mail_list', array.values($unique_email), $board, $wr_id);
...
if($bo_table == 'inquiry') alert('문의가 접수되었습니다. 감사합니다.');
