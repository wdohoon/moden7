...
// 총 지급된 포인트
$sql = " select sum(po_point) as total_given_points from {$g5['point_table']} where po_point > 0 ";
$row = sql_fetch($sql);
$total_given_points = $row['total_given_points'];

// 소멸된 포인트
$sql = " select sum(po_point) as total_lost_points from {$g5['point_table']} where po_point < 0 ";
$row = sql_fetch($sql);
$total_lost_points = $row['total_lost_points'];

// 현재 보유 포인트
$sql = " select sum(mb_point) as current_points from {$g5['member_table']} ";
$row = sql_fetch($sql);
...
// 회원 목록 SQL 쿼리
$sql = " select m.*, k.wr_id, k.wr_homepage 
         {$sql_common}
         left join {$g5['write_prefix']}kyc k on m.mb_id = k.mb_id 
         {$sql_search} 
         {$sql_order} 
...
<div class="local_ov01 local_ov">
    ...
    <span class="btn_ov01"><span class="ov_txt">총 지급된 포인트 </span><span class="ov_num"> <?php echo number_format($total_given_points) ?>점 </span></span>
    <span class="btn_ov01"><span class="ov_txt">총 소멸된 포인트 </span><span class="ov_num"> <?php echo number_format($total_lost_points) ?>점 </span></span>
    <span class="btn_ov01"><span class="ov_txt">현재 보유 포인트 </span><span class="ov_num"> <?php echo number_format($current_points) ?>점 </span></span>
	<span class="btn_ov01"><a href="/bbs/board.php?bo_table=kyc"><span class="ov_num">kyc 인증</span></a></span>
	<span class="btn_ov01"><a href="/adm/contentform.php?w=u&co_id=privacy"><span class="ov_num">개인정보 처리 방침</span></a></span>
	<span class="btn_ov01"><a href="/adm/contentform.php?w=u&co_id=terms"><span class="ov_num">회원가입약관</span></a></span>
</div>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col" id="mb_list_mailc"><?php echo subject_sort_link('mb_email_certify', '', 'desc') ?>약관 동의</a></th>
        <!-- <th scope="col" id="mb_list_open"><?php echo subject_sort_link('mb_open', '', 'desc') ?>정보공개</a></th> -->
        <th scope="col" id="mb_list_mailr"><?php echo subject_sort_link('mb_mailling', '', 'desc') ?>이메일</a></th>
        <th scope="col" id="mb_list_auth">상태</th>
        <th scope="col" id="mb_list_mobile">휴대폰</th>
        <th scope="col" id="mb_list_lastcall"><?php echo subject_sort_link('mb_today_login', '', 'desc') ?>최종접속</a></th>
        <th scope="col" id="mb_list_grp">kyc 인증</th>
        <th scope="col" rowspan="2" id="mb_list_mng">관리</th>
    </tr>
    <tr>
        <th scope="col" id="mb_list_name"><?php echo subject_sort_link('mb_name') ?>이름</a></th>
        <th scope="col" id="mb_list_nick"><?php echo subject_sort_link('mb_nick') ?>성별</a></th>
        <th scope="col" id="mb_list_sms"><?php echo subject_sort_link('mb_sms', '', 'desc') ?>핀번호</a></th>
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td headers="mb_list_mailr">
			<?php echo $row['mb_7'];?>@<?php echo $row['mb_8'];?>
        ...
        <td headers="mb_list_grp" class="td_numsmall">
            <?php 
            if ($row['wr_homepage'] == '1') {
                echo '<a href="https://bhidex.gabia.io/bbs/board.php?bo_table=kyc&wr_id=' . $row['wr_id'] . '">승인 완료</a>';
            } else if ($row['wr_homepage'] == '2') {
                echo '<a href="https://bhidex.gabia.io/bbs/board.php?bo_table=kyc&wr_id=' . $row['wr_id'] . '">승인 대기 중</a>';
            } else {
                echo '<a href="https://bhidex.gabia.io/bbs/board.php?bo_table=kyc&wr_id=' . $row['wr_id'] . '">미신청</a>';
            }
            ?>
        </td>
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td headers="mb_list_nick" class="td_name sv_use"><div><?php echo get_text($row['mb_sex']); ?></div></td>
        
        <td headers="mb_list_sms">
			<?php echo $row['mb_1']?>
            <!-- <label for="mb_sms_<?php echo $i; ?>" class="sound_only"><?php echo $row['mb_1']?></label>
