...
add_javascript('<script src="'.G5_JS_URL.'/plugin/jquery-ui.min.js"></script>', 0);
add_javascript('<script src="'.G5_JS_URL.'/front.js"></script>', 0);
if (defined('_SHOP_')) {
    if(!G5_IS_MOBILE) {
...
add_stylesheet('<link rel="stylesheet" href="'.G5_CSS_URL.'/front.css">', 0);
...
$chwpath = trim($_SERVER['REQUEST_URI']);
if(!$member['mb_id'] && (strpos($chwpath,"/login.php") !== false) ){//로그인이 아니고 로그인화면 이면


}else if((!$member['mb_id'] && strpos($chwpath,"login_check") !== false)){

}else if((!$member['mb_id'] && strpos($chwpath,"register") !== false)){

}else if(!$member['mb_id'] && strpos($chwpath,"password_change.php") !== false){

}else if(!$member['mb_id']){

//goto_url("/bbs/login.php?url=".$chwpath);
}
?>
