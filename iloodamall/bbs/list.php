...
if($bo_table=="marketing"){	//제품정보 게시판일때.
		
	//if ($sca || $stx || $stx === '0' || $sch) {
	if ($stx || $stx === '0' || $sch) {     //검색이면($sca는 카테고리이기에 검색일때'만' 조건에서는 뺐다)
		...
		if($sch=='all'){
			$sql_search .= " and (wr_subject like '%{$stx}%' or wr_1 like '%{$stx}%' or wr_2 like '%{$stx}%' or wr_3 like '%{$stx}%' or wr_4 like '%{$stx}%' or wr_5 like '%{$stx}%' or wr_6 like '%{$stx}%' or wr_7 like '%{$stx}%' or wr_8 like '%{$stx}%' or wr_9 like '%{$stx}%' or wr_10 like '%{$stx}%') ";
		}else if($sch=='wr_subject'){
			$sql_search .= " and (wr_subject like '%{$stx}%') ";
		}else if($sch=='wr_5'){
			$sql_search .= " and (wr_5 like '%{$stx}%') ";	
		}else if($sch=='wr_4'){
			$sql_search .= " and (wr_4 like '%{$stx}%') ";
		}
	...
	}else if($wr_1){	//2차분류 클릭했을때
		$sql_search = "ca_name = '{$sca}'";

		$sql_search .= " and wr_1 like '%{$wr_1}%'";	
		$sql = " SELECT COUNT(DISTINCT `wr_parent`) AS `cnt` FROM {$write_table} WHERE {$sql_search} ";
		$row = sql_fetch($sql);
		$total_count = $row['cnt'];

	}else if(!$wr_1 && !$sch && $sca){		//카테고리만 있을때
		$sql_search = "ca_name = '{$sca}'";

		$sql = " SELECT COUNT(DISTINCT `wr_parent`) AS `cnt` FROM {$write_table} WHERE {$sql_search} ";
		$row = sql_fetch($sql);
		$total_count = $row['cnt'];
	}else if(!$wr_1 && !$sch && !$sca){		//카테고리도없고 전부싹다없을때
		$sql_search = '';
...
if ($is_search_bbs) {
	...
	if($bo_table=='marketing'){
		if($wr_1 && $sca){
			$sql .= " and ca_name='{$sca}' and wr_1 like '%{$wr_1}%' ";
		}else if(!$wr_1 && $sca){
			$sql .= " and ca_name='{$sca}' ";
		}else if(!$wr_1 && !$sca){
	...
	if($bo_table=='inquiry_details'){
		$sql = "select * from g5_write_inquiry where wr_is_comment = 0  and wr_name = '{$myname}' and wr_6 = '{$myemail}'";
	}
