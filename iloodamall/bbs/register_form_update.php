...
if ($w == '') {
    ...
    // 추천인에게 적립금 부여, 추천등록한 본인도 적립금 부여
    if ($config['cf_use_recommend'] && $mb_recommend){
        insert_point2($mb_recommend, $config['cf_recommend_money'], $mb_id.'의 추천인', '@member', $mb_recommend, $mb_id.' 가 추천함');
        insert_point2($mb_id, $config['cf_recommend_money'], $mb_recommend.'를 추천인등록', '@member', $mb_id, $mb_recommend.' 를 추천등록함');
	}
...
if($bo_table == 'genuine' && $input1 && $inputtext1){
...
$wr_name = $mb_name;
$wr_subject = $mb_id ."님의 정품등록신청";
$wr_content ="정품등록 신청입니다.";
...
$wr_2 = $inputtext1;
		...
		$sql_h = sql_fetch(" select * from genuine_list where mb_id = '{$mb_id}' and pd_name = '{$wr_1}' and pd_num = '{$wr_2}'  ");		
		
		if($sql_h){
			alert($wr_1[$v].' / '.$wr_2[$v].'는 이미 등록한 값입니다.');
		}

		$ssql = " select * from g5_write_serial where wr_1 = '{$wr_2}' ";
		$fet = sql_fetch($ssql);
		
		if($wr_2 == $fet['wr_1']){	//사용자가 등록한 값들이 실제 시리얼넘버랑 겹치면

			$arr[] = $wr_2;

			$arr = array_unique(array_values(array_filter($arr)));

			insert_point2($mb_id, $po_point, $po_content, '@passive', $mb_id, $mb_id . '-' . uniqid(''), $expire);
		...
		$sql_kkk = "select * from genuine_list where pd_name = '{$wr_1}' and pd_num = '{$wr_2}' order by no desc ";
		...
		sql_query($sql);
		 ...
		 $sql = " insert into g5_write_genuine
                set wr_num = '-80',
                     wr_reply = '$wr_reply',
                     wr_comment = 0,
                     ca_name = '$ca_name',
...
if ($w == '') {
	if($action=='complete'){
		...
		goto_url(G5_HTTP_BBS_URL.'/survey.php');
	}
} else if ($w == 'u') {
