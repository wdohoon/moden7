...
/* 적립금 */
// 적립금 부여
function insert_point2($mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0)
{
    global $config;
    global $g5;
    global $is_admin;

    // 적립금 사용을 하지 않는다면 return
    //if (!$config['cf_use_point']) { return 0; }

    // 적립금가 없다면 업데이트 할 필요 없음
    if ($point == 0) { return 0; }
    ...
    if($config['cf_money_term'] > 0) {
        if($expire > 0)
            ...
            $po_expire_date = date('Y-m-d', strtotime('+'.($config['cf_money_term'] - 1).' days', G5_SERVER_TIME));
    }
...
// 사용적립금 입력
function insert_use_point2($mb_id, $point, $po_id='')
{
    global $g5, $config;
...
// 사용적립금 삭제
function delete_use_point2($mb_id, $point)
{
    global $g5, $config;
...
// 소멸적립금 삭제
function delete_expire_point2($mb_id, $point)
{
    global $g5, $config;
...
// 적립금 내역 합계
function get_point_sum2($mb_id)
{
    global $g5, $config;

    if($config['cf_money_term'] > 0) {
        // 소멸적립금가 있으면 내역 추가
        $expire_point = get_expire_point2($mb_id);
        if($expire_point > 0) {
            $mb = get_member($mb_id, 'mb_point2');
            $content = '적립금 소멸';
            $rel_table = '@expire';
            $rel_id = $mb_id;
            $rel_action = 'expire'.'-'.uniqid('');
            $point = $expire_point * (-1);
            $po_mb_point = $mb['mb_point2'] + $point;
            $po_expire_date = G5_TIME_YMD;
            $po_expired = 1;
...
// 소멸 적립금
function get_expire_point2($mb_id)
{
    global $g5, $config;
...
// 적립금 삭제
function delete_point2($mb_id, $rel_table, $rel_id, $rel_action)
{
    global $g5;
    ...
    {
        // 적립금 내역정보
        $sql = " select * from {$g5['point2_table']}
                    where mb_id = '$mb_id'
                      and po_rel_table = '$rel_table'
                      and po_rel_id = '$rel_id'
                      and po_rel_action = '$rel_action' ";
        ...
        if(isset($row['po_point']) && $row['po_point'] < 0) {
            ...
            delete_use_point2($mb_id, $po_point);
        } else {
            if(isset($row['po_use_point']) && $row['po_use_point'] > 0) {
                insert_use_point2($row['mb_id'], $row['po_use_point'], $row['po_id']);
            }
        }

        $result = sql_query(" delete from {$g5['point2_table']}
                     where mb_id = '$mb_id'
                       and po_rel_table = '$rel_table'
                       and po_rel_id = '$rel_id'
                       and po_rel_action = '$rel_action' ", false);
        ...
        // 적립금 내역의 합을 구하고
        $sum_point = get_point_sum2($mb_id);

        // 적립금 UPDATE
        $sql = " update {$g5['member_table']} set mb_point2 = '$sum_point' where mb_id = '$mb_id' ";
        $result = sql_query($sql);
    }
