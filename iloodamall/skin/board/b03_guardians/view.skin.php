...
<style>
#bo_v_con ul#option_list {padding-left:0;}
#option_list:after {display:block;visibility:hidden;clear:both;content:""}
#option_list li {display:block;margin-bottom:10px;}
</style>
...
<?php
$mmb = sql_fetch(" select * from g5_member where mb_id = '{$view['mb_id']}' ");
$genuinemb = sql_fetch(" select * from g5_write_genuine where mb_id = '{$view['mb_id']}' and wr_datetime <= '{$view['wr_datetime']}' ");
?>
        ...
        <div id="bo_v_con">		
			<div class="jtbl_frm01 tbl_wrap" style="margin-bottom: 0;">
				...
				<?php if($mmb['mb_id']){ ?>
				<tr>
					<th scope="row"><label for="mb_id">병원</label></th>
					<td>
						<?php echo $mmb['mb_1'] ?>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="mb_id">정품등록여부</label></th>
					<?php if($genuinemb['mb_id']){ ?>
					<td>O</td>
					<?php }else{ ?>
					<td>X</td>
					<?php } ?>
				...
				<tr>
					<th scope="row"><label for="wr_subject">연락처</label></th>
					<td>
						<?php echo $view['wr_2'] ?>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="wr_datetime">신청일시</label></th>
					<td>
				...
				<tr>
					<th scope="row"><label for="wr_1">강의 일정</label></th>
					<td>
						<?php
				...
				<tr>
					<th scope="row"><label for="wr_7">강의 제목</label></th>
					<td>
						<?php
						$option7 = explode('|', $view['wr_7']);
						for ($j=0; $j<count($option7); $j++) {
							$option7_list = trim($option7[$j]);
							if ($option7_list=='') continue;
							$option_view7 .= '<li>'.$option7_list.'</li>';
						}
						?>
						<ul id="option_list">
							<?php echo $option_view7; ?>
						</ul>
					</td>
				</tr>

				<tr>
					<th scope="row"><label for="wr_8">강사</label></th>
					<td>
						<?php
						$option8 = explode('|', $view['wr_8']);
						for ($k=0; $k<count($option8); $k++) {
							$option8_list = trim($option8[$k]);
							if ($option8_list=='') continue;
							$option_view8 .= '<li>'.$option8_list.'</li>';
						}
						?>
						<ul id="option_list">
							<?php echo $option_view8 ?>
						</ul>
					</td>
				</tr>

				<tr>
					<th scope="row"><label for="wr_9">강연 장소</label></th>
					<td>
						<?php
						$option9 = explode('|', $view['wr_9']);
						for ($l=0; $l<count($option9); $l++) {
							$option9_list = trim($option9[$l]);
							if ($option9_list=='') continue;
							$option_view9 .= '<li>'.$option9_list.'</li>';
						}
						?>
						<ul id="option_list">
							<?php echo $option_view9 ?>
						</ul>
					</td>
				</tr>

				<tr>
					<th scope="row"><label for="wr_content">도시락 신청여부</label></th>
					<td>
						<?php echo $view['wr_19'] ?>
					</td>
				</tr>
				...
				<tr>
					<th scope="row"><label for="wr_content">가디언즈 기념품 신청여부</label></th>
					<td>
						<?php echo $view['wr_20'] ?>
					</td>
				</tr>
				...
				<tr>
					<th scope="row"><label for="wr_content">소속</label></th>
					<td>
						<?php echo $view['wr_3'] ?>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="wr_subject">리팟에 대한 궁금한 점이 있나요? </label></th>
					<td>
						<?php echo $view['wr_4'] ?>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="wr_content">리팟 가디언즈 프로그램에 대한 기타 의견이 있으시면 알려주세요!</label></th>
					<td>
						<?php echo $view['wr_5'] ?>
					</td>
				</tr>
