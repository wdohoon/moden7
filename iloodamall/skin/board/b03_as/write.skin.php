...
<style>
.jtbl_frm01 select{max-width: 355px; min-width: 180px; width: 100%; color:#000; height: 43px; padding-left: 15px;}
.req { color: #568fee; }
...
@media (max-width: 800px) {
	.jtbl_frm01 select{height: 32px; padding-left: 10px;}
...
<section id="bo_w">
    ...
    <div class="jtbl_frm01 tbl_wrap">
        ...
        <?php } ?>
		<tr>
            <th scope="row"><label for="wr_3">병원명 <span class="req">*</span><strong class="sound_only">필수</strong></label></th>
            <td><input type="text" name="wr_3" value="<?php if($w==''){ echo $member['mb_1']; }else{ echo $write['wr_3']; } ?>" id="wr_3" required class="frm_input" size="10" maxlength="30" style="max-width: 355px;"></td>
        </tr>
        <tr>
            <th scope="row"><label for="wr_name">이름 <span class="req">*</span></label></th>
            <td>
				<input type="text" name="wr_name" value="<?php if($w==''){ echo $member['mb_name']; }else{ echo $write['wr_name']; } ?>" id="wr_name" required class="frm_input" style="max-width: 355px;">
			</td>
        </tr>
...
<!-- 		<tr>
		...
		<?php }  ?>
		<tr>
            <th scope="row"><label for="wr_email">이메일 <span class="req">*</span></label></th>
            <td><input type="text" name="wr_email" value="<?php if($w==''){ echo $member['mb_email']; }else{ echo $write['wr_email']; } ?>" id="wr_email" required class="frm_input email" style="max-width: 355px;"></td>
        </tr>
		
		<tr>
            <th scope="row"><label for="wr_subject">제목<span class="req">*</span><strong class="sound_only">필수</strong></label></th>
            <td>
                <div id="autosave_wrapper">
                    <input style="max-width:355px;min-width:180px;;" type="text" name="wr_subject" value="<?php echo $subject ?>" id="wr_subject" required class="frm_input"  maxlength="255">
		...
		<tr>
            <th scope="row"><label for="wr_1">제품 종류 <span class="req">*</span></label></th>
			<td>
				<select name="wr_1" id="">
            ...
            <!-- <td class="td_job">
            				<?php 
            					...
            					$sql = "select distinct pd_name from genuine_list where mb_id='{$member['mb_id']}' ";
            				...
            				?>
            				<span>
            					<input type="radio" name="wr_1" value="<?php echo $rows['wr_1']; ?>" <?php if($wr_1==$rows['wr_1'])echo"checked";?> id="job1" required>
            					<label for="job1"><?php echo $rows['wr_1']; ?></label> &nbsp;&nbsp;
            				</span>
            				<?php 
            					if($row){ ?>
            					<select name="wr_1" id="">
            					...
            					?>
            						<option value="<?php echo $row['pd_name']; ?>"><?php echo $row['pd_name']; ?></option>
            					<?php } ?>
            					</select>
            				<?php 
            					} 
		...
		<tr>
            <th scope="row"><label for="wr_2">연락처 <span class="req">*</span></label></th>
				<td>
					<input type="text" name="wr_2" value="<?php if($w==''){ echo $member['mb_hp']; }else{ echo $write['wr_2']; } ?>" id="wr_2" required class="frm_input" style="max-width: 355px;" >
				</td>
        </tr>
    ...
    <div class="write_btn_div">
		<?php if($is_admin) { ?>
        <a href="./board.php?bo_table=<?php echo $bo_table ?>" class="btn_cancel btn" style="background:#b1b4b9 !important">목록보기</a>
		<?php } ?>
        <input type="submit" value="작성완료" id="btn_submit" accesskey="s" class="btn_submit btn" style="background:#b1b4b9 !important">
    </div>
    </form>
