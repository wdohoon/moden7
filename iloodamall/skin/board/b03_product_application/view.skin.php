...
if(!$is_admin){
	if($is_member){
		if($view['mb_id'] == $member['mb_id']){
			...
			echo "<script>alert('내가 쓴 게시글이 아닙니다.'); location.href='/bbs/board.php?bo_table=product_application'</script>";
		}
	} else {
		echo "<script>alert('로그인하십시오.'); location.href='/index.php'</script>";
	}
}
...
<article id="bo_v" style="width:<?php echo $width; ?>">
    ...
    <section id="bo_v_atc">
		...
		<?php
		// 게시글의 작성자 mb_id를 사용하여 g5_member 테이블에서 정보 조회
		...
		$member_name = $member_result['mb_name'];
		?>
        <!-- 본문 내용 시작 { -->
        <div id="bo_v_con">	
			<div class="jtbl_frm01 tbl_wrap" style="margin-bottom: 0;">
				...
				<tr>
					<th scope="row"><label for="wr_name">등록일</label></th>
					<td>
						<?php echo date("Y-m-d H:i", strtotime($view['wr_datetime'])) ?>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="mb_name">성함</label></th>
					<td>
...
<!-- 				<tr>
				...
				<tr>
					<th scope="row"><label for="wr_1">문의 종류</label></th>
					<td>
				...
				<tr>
					<th scope="row"><label for="wr_4">신청 상품</label></th>
					<td>
						<?php 
						$sql = "select * from g5_shop_item where it_id = '{$view['wr_4']}'";
						$row = sql_fetch($sql);
						echo "<a href='/shop/item.php?it_id={$view['wr_4']}'> {$row['it_name']} </a>"; ?>
					</td>
				</tr>
				<?php if($bo_table =='product_application'){ ?>
				<tr>
					<th scope="row">설치 희망기간</th>
					<td>
						<?php echo $view['wr_8'] ?> ~ <?php echo $view['wr_9'] ?>
					</td>
				</tr>
				<?php } ?>
				<?php }else{ ?>
				...
				<tr>
					<th scope="row"><label for="wr_1">제품명</label></th>
					<td>
						<?php echo get_text($view['wr_1']) ?>
					</td>
				</tr>
				...
				<?php 
					...
					?>
					<tr>
						<th scope="row"><label for="wr_2">시리얼넘버 no.<?php echo $i+1?></label></th>
						<td>
							<?php echo $wr_2[$i] ?> 
							<span>
