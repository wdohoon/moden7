...
// 비교할 범위 설정
$startDateTime = new DateTime('2023-12-22 14:00:00');
$endDateTime = new DateTime('2024-01-02 09:00:00');
...
else if ($od_settle_case == "계좌이체")
{
    switch($default['de_pg_service']) {
        ...
        case 'nicepay': // acefive
            include G5_SHOP_PATH.'/nicepay/pay_hub.php';
            break;
        default:
    ...
    if((int)$order_price !== (int)$pg_price) {
        ...
        switch($od_pg) {
            ...
            case 'KAKAOPAY':
                ...
                break;
            case 'nicepay': // acefive
                include G5_SHOP_PATH.'/nicepay/pay_cancel.php';
                break;
            default:
...
/* chw chw ace */
if($od_pg=="nicepay") { // 나이스페이 복합과세
    $od_tax_mny = isset($_POST['SupplyAmt']) ? (int) $_POST['SupplyAmt'] : 0;
    $od_vat_mny = isset($_POST['GoodsVat']) ? (int) $_POST['GoodsVat'] : 0;
    $od_free_mny = isset($_POST['TaxFreeAmt']) ? (int) $_POST['TaxFreeAmt'] : 0;	
}
/* chw chw ace */
...
$od_cart_price = number_format($tot_ct_price);

if($od_settle_case == '무통장'){
	include_once('../kakao/order_ok.php');
} else {
	include_once("../kakao/deposit_ok.php");
}
