...
#smb_my_ov{margin:20.7119vw 0 2.8249vw;}
...
<div id="smb_my">
	<!-- 정품인증회원만 노출 -->
	<?php 
		$mygenuine = sql_fetch("select * from genuine_list where mb_id = '{$member['mb_id']}'");
		if($mygenuine['mb_id']){ 
	?>
	<section id="genuine" style="display:none;">
		<h3>정품등록이 완료되었습니다.</h3>
		<h4 class="<?php if($i == ($sncnt-1)){ echo 'last'; } ?>">
		<?php 
			$sql = "select * from genuine_list where mb_id = '{$member['mb_id']}' and (division = 0 or division = 2)";
		...
		<span><?php echo "- ".$pd_name; ?>&nbsp;&nbsp;|&nbsp;&nbsp;<?php echo  $pd_nums_str; ?></span><br>
		<?php } ?>
		</h4>
		<ul>
    ...
    <section id="smb_my_ov">
        ...
        <dl class="op_area">
	     ...
	     <dt id="">주소</dt>
            <dd id=""><?php echo sprintf("(%s%s)", $member['mb_zip1'], $member['mb_zip2']).' '.print_address($member['mb_addr1'], $member['mb_addr2'], $member['mb_addr3'], $member['mb_addr_jibeon']); ?></dd>
	    </dl>
	     <dl class="op_area">
	    <?php 
			$mygenuine = sql_fetch("select * from genuine_list where mb_id = '{$member['mb_id']}'");
		...
		?>
			...
			<dt><?php echo $pd_name; ?></dt>
			<dd><?php echo  $pd_nums_str; ?></dd>
			<?php } ?>
		<?php }?>
        </dl>
...
<!-- 빠른문의내역 시작 { -->
<section id="smb_my_wish">
    <h3 style="font-size: 1.25em;padding: 0 0 20px;">1:1문의내역</h3>
<!--     <ul>
        <?php
        $sql = "SELECT * FROM g5_write_inquiry WHERE mb_id = '{$member['mb_id']}' ORDER BY wr_id DESC LIMIT 5";
        $result = sql_query($sql);
    
        while ($row = sql_fetch_array($result)) {
            echo '<li>';
            echo '<strong>'.htmlspecialchars($row['wr_subject']).'</strong>'; // 제목 출력
            echo '<p>'.nl2br(htmlspecialchars($row['wr_content'])).'</p>'; // 내용 출력
            echo '<span>'.date('Y-m-d', strtotime($row['wr_datetime'])).'</span>'; // 날짜 출력
            echo '</li>';
        }
        ?>
    		<?php
        $sql = "SELECT * FROM g5_write_mb_inquiry WHERE mb_id = '{$member['mb_id']}' ORDER BY wr_id DESC LIMIT 5";
        $result = sql_query($sql);
...
<!-- 문의 내역 폼 -->
	...
	<?php
	// g5_write_inquiry와 g5_write_mb_inquiry에서 문의 내역 조회를 위한 SQL 쿼리
	$sql = "(SELECT wr_id, wr_subject, wr_name, wr_datetime FROM g5_write_inquiry WHERE mb_id = '{$member['mb_id']}')
			 UNION
			 (SELECT wr_id, wr_subject, wr_name, wr_datetime FROM g5_write_mb_inquiry WHERE mb_id = '{$member['mb_id']}')
			 ORDER BY wr_datetime DESC";
	$result = sql_query($sql);

	// 테이블 헤더
	echo '<div class="tbl_head03 tbl_wrap">';
	echo '<table>';
	echo '<thead>';
	echo '<tr>';
	echo '<th scope="col">NO</th>';
	echo '<th scope="col" class="th02" style="width:50%;">제목</th>';
	echo '<th scope="col" class="th01">이름</th>';
	echo '<th scope="col" class="th03">등록일</th>';
	echo '</tr>';
	echo '</thead>';
	echo '<tbody>';
	...
	for ($i=0; $row=sql_fetch_array($result); $i++) {
		// 문의 상세 페이지 URL (조정 필요)
		// 상세 페이지 링크가 다를 경우, 링크 로직을 수정해야 할 수 있음
		$detail_url = '/bbs/board.php?bo_table=inquiry&wr_id='.$row['wr_id'];

		echo '<tr>';
		echo '<td>'.($i+1).'</td>'; // 번호
		echo '<td class="td_subject"><a href="'.$detail_url.'">'.htmlspecialchars($row['wr_subject']).'</a></td>'; // 제목에 링크 추가
		echo '<td>'.htmlspecialchars($row['wr_name']).'</td>'; // 이름
		echo '<td>'.date('Y-m-d', strtotime($row['wr_datetime'])).'</td>'; // 등록일
		echo '</tr>';
	}
...
<!--     <div class="smb_my_more">
        <a href="/bbs/board.php?bo_table=inquiry">1:1문의 더보기</a>
    </div> -->
</section>
<!-- 빠른문의내역 끝 -->
