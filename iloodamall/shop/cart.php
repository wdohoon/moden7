...
if (!$is_member)
    goto_url(G5_BBS_URL."/login.php?url=".urlencode(G5_SHOP_URL."/cart.php"));
...
<div id="sod_bsk" class="od_prd_list">
	...
	<div class="ct_mo">
		...
		<ul class="sod_list">
			...
			<li class="sod_li sod_it_id">
				<input type="hidden" name="it_id[<?php echo $i; ?>]"    value="<?php echo $row['it_id']; ?>">
				<input type="hidden" name="it_name[<?php echo $i; ?>]"  value="<?php echo get_text($row['it_name']); ?>">

				<div class="li_op_wr">
					<div class="li_chk chk_box">
						<input type="checkbox" name="ct_chk2[<?php echo $i; ?>]" value="1" id="mo_ct_chk_<?php echo $i; ?>" class="selec_chk" checked chk-device="mo">
						<label for="mo_ct_chk_<?php echo $i; ?>"><span></span><b class="sound_only">상품선택</b></label>
					</div> 
