...
<div id="smb_my">
	<!-- 정품인증회원만 노출 기존 -->
	<?php 
		$genuinesql = "select * from g5_write_genuine where mb_id = '{$member['mb_id']}'";
		$mygenuine = sql_fetch("select * from g5_write_genuine where mb_id = '{$member['mb_id']}'");
		$genuineresult = sql_query($genuinesql);
	...
	<section id="genuine">
		...
		<?php 
			...
			for($i = 0 ; $i < $sncnt ; $i++) {		//총합은 시리얼넘버 갯수로 통일
		?>

		<span><?php echo "- ".$product[$i]; ?>&nbsp;&nbsp;|&nbsp;&nbsp;<?php echo $sn[$i]; ?></span><br>
		<?php } ?>
		</h4>
		<ul>
	...
	<section id="genuine">
		...
		<?php 
			...
			// 결과 출력
			foreach ($resultGenuine as $wr_subject => $values) {
		?>
