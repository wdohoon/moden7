...
if (($i_temp_point > (int)$temp_point || $i_temp_point < 0) && $config['cf_use_money']) {
...
if ($od_temp_point)
{
    if ($member['mb_point2'] < $od_temp_point) {
...
$sql_card_point = "";
if ($od_receipt_price > 0 && !$default['de_card_money']) {
...
if ($is_member && $od_receipt_point)
    insert_point2($member['mb_id'], (-1) * $od_receipt_point, "주문번호 $od_id 결제");
...
goto_url('/creative-marketing/orderinquiryview.php?od_id='.$od_id.'&amp;uid='.$uid);
?>
<html>
    <head>
        <title>주문정보 기록</title>
