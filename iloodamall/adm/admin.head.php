...
function print_menu2($key, $no = '')
{
    ...
    for ($i = 1; $i < count($menu[$key]); $i++) {
		...
		if ($menu[$key][$i][0] == '300110' || $menu[$key][$i][0] == '300130') {
			$str .= '<li data-menu="'.$menu[$key][$i][0].'"><a href="'.$menu[$key][$i][2].'" class="gnb_2da '.$gnb_grp_style.' '.$gnb_grp_div.$current_class.'" target="_blank">'.$menu[$key][$i][1].'</a></li>';
		} else {
...
<header id="hd">
    ...
    <nav id="gnb" class="gnb_large <?php echo $adm_menu_cookie['gnb']; ?>">
        ...
        <ul class="gnb_ul">
			...
			<?php
			// 현재 로그인한 사용자가 최고관리자이거나 아이디가 'as_admin'인 경우에만 메뉴를 보여줍니다.
			if ($member['mb_id'] == 'admin' || $member['mb_id'] == 'as_admin') {
			?>
				<li class="gnb_li">
					<a href="/bbs/board.php?bo_table=as" class="btn btn_02" style="width: 100%; text-align:center; padding:0;">A/S</a>
					<a href="/bbs/board.php?bo_table=serial" class="btn btn_02" style="width: 100%; text-align:center; padding:0;">Serial</a>
				</li>
			<?php
			}
			?>
