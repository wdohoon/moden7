...
$fr_date = isset($_REQUEST['fr_date2']) ? preg_replace('/[^0-9 :_\-]/i', '', $_REQUEST['fr_date2']) : '';
$to_date = isset($_REQUEST['to_date2']) ? preg_replace('/[^0-9 :_\-]/i', '', $_REQUEST['to_date2']) : '';
...
if ($csv == 'xls')
{
    ...
    $sql = " SELECT a.od_id, a.od_name, a.od_settle_case, a.od_receipt_time, a.od_receipt_price, a.od_cart_price, b.ct_price, a.od_deposit_name, a.od_receipt_point, a.od_time, a.mb_id, od_b_zip1, od_b_zip2, od_b_addr1, od_b_addr2, od_b_addr3, od_b_addr_jibeon, od_b_name, a.od_status, a.od_email, od_b_tel, od_b_hp, b.it_name, ct_qty, b.it_id, a.od_id, od_memo, od_invoice, b.ct_option, b.io_id, b.ct_send_cost, b.it_sc_type
               FROM {$g5['g5_shop_order_table']} a, {$g5['g5_shop_cart_table']} b
              where a.od_id = b.od_id ";
    if ($case == 1) // 출력기간
        $sql .= " and a.od_time between '$fr_date 00:00:00' and '$to_date 23:59:59' ";
    ...
    if (!$cnt)
        ...
        $headers = array('주문 번호', '회원명', '상호', '사업자번호', '총 공급가', '총 부가세', '결제방법', '상품명', '상품수량', '실 입금일자', '주문일자', '수취인 이름', '수취인 전화번호', '수취인 핸드폰번호', '수취인 우편번호', '수취인 주소', '회원 아이디', '총 결제금액', '사용된 포인트', '입금자', '입금일자', '주문자 이메일', '주문자 전화번호', '주문자 핸드폰번호', '주문상태', '판매가', '소계');
        $widths  = array(20, 15, 20, 15, 10, 10, 10, 40, 10, 15, 15, 10, 20, 20, 20, 50, 15, 15, 15, 15, 15, 15, 15, 15, 10, 10, 10);
        $header_bgcolor = 'FFABCDEF';
        //$last_char = column_char(count($headers) - 1);
		$last_char = 'AA'; //열의 총 갯수만큼 현재 27개의 내용이 출력되어야 하니까 A열부터 AA열까지 출력해야함으로 마지막 열을 지정해줌
        for($i=1; $row=sql_fetch_array($result); $i++) {
			$memmem = sql_fetch(" select * from g5_member where mb_id = '{$row['mb_id']}' ");

			//총금액
			$resultpri = intval($row['od_receipt_price']);
			
			//소계
			$allpri = (int) $row['ct_price'] * (int) $row['ct_qty'];

			//부가세
			$pritwo = intval(round(($resultpri * 10) / 110));

			//공급가
			$prione = intval($resultpri - $pritwo);
            ...
            $rows[] = array(' '.$row['od_id'],
							...
							$pritwo,
							$row['od_settle_case'],
							preg_replace("/\"/", "&#034;", $row['it_name']).' '.$row['io_id'],
							' '.$row['ct_qty'],
							$row['od_receipt_time'],
							$row['od_time'],
							$row['od_b_name'],
							...
							$row['od_receipt_time'],
							$row['od_email'],
							' '.conv_telno($row['od_tel']), 
                            ' '.conv_telno($row['od_hp']),
							$row['od_status'],
							$row['od_cart_price'],
							$allpri);
        }
