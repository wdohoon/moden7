...
{
    $p_ca_name = isset($_POST['ca_name'][$i]) ? strip_tags(clean_xss_attributes($_POST['ca_name'][$i])) : '';
    $ca_id = isset($_POST['ca_id'][$i]) ? clean_xss_tags($_POST['ca_id'][$i]) : '';

    // 각 카테고리 항목 업데이트
    $sql = "UPDATE {$g5['g5_shop_category_table']}
            SET ca_name = '".sql_real_escape_string($p_ca_name)."'
            WHERE ca_id = '".sql_real_escape_string($ca_id)."'";
    sql_query($sql);
    ...
    $log_sql = "INSERT INTO g5_login_history_save
                (login_id, user_name, user_ip, accessed_page, history_datetime, action, target_id)
                VALUES
                ('{$login_id}', '{$mb_name}', '{$user_ip}', '{$accessed_page}', '{$history_datetime}', '{$action}', '{$ca_id}')";
    sql_query($log_sql);
}
