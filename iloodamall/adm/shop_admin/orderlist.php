...
$sel_field = (isset($_GET['sel_field']) && in_array($_GET['sel_field'], array('od_id', 'mb_id', 'od_name', 'od_tel', 'od_hp', 'od_b_name', 'od_b_tel', 'od_b_hp', 'od_deposit_name', 'od_invoice', 'hospital')) ) ? $_GET['sel_field'] : ''; 
...
if($sel_field == 'hospital'){
	$sql = "select g5_shop_order.*, (od_cart_coupon + od_coupon + od_send_coupon) as couponprice 
			from g5_shop_order
			join g5_member on g5_shop_order.mb_id = g5_member.mb_id
			where g5_member.mb_1 like '%$search%'
			order by $sort1 $sort2
			limit $from_record, $rows;";

			//select *, (od_cart_coupon + od_coupon + od_send_coupon) as couponprice from g5_shop_order order by od_id desc limit 0, 15
}
...
<select name="sel_field" id="sel_field">
    ...
    <option value="hospital" <?php echo get_selected($sel_field, 'hospital'); ?>>소속병원</option>
...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        ...
        if ($row['od_receipt_point'] > 0){
			//적립금 사용여부 확인
			$sql_i = "select g5_shop_item.ca_id from g5_shop_cart join g5_shop_item on g5_shop_cart.it_id = g5_shop_item.it_id where g5_shop_cart.od_id = '{$row['od_id']}'";
			$row_i = sql_fetch($sql_i);
			if($row_i['ca_id'] == 40){
				$s_receipt_way .= $s_br."적립금";
			} else {
...
<div class="btn_fixed_top">
	<form name="forderprint" action="./orderprintresult2.php" onsubmit="return forderprintcheck(this);" autocomplete="off">
		<a href="./orderprintresult3.php" class="btn_submit" style="display: inline-block; padding:9px 10px; border:2px solid black;">주문내역 전체출력</a>
        <input type="hidden" name="case" value="1">
        <input type="hidden" name="csv" value="xls">

        <!-- <input type="radio" name="csv" value="xls" id="xls1" checked>
        ...
        <input type="text" name="fr_date2" value="<?php echo date("Ymd"); ?>" id="fr_date2" required class="required frm_input" size="10" maxlength="8">
        ~
        <label for="to_date2" class="sound_only">기간 종료일</label>
        <input type="text" name="to_date2" value="<?php echo date("Ymd"); ?>" id="to_date2" required class="required frm_input" size="10" maxlength="8">
        <input type="submit" value="출력 (새창)" class="btn_submit" style="height:35px; padding:0 10px;">
