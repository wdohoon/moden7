...
$it_id = isset($_POST['it_id']) ? preg_replace('/[^0-9a-z]/i', '', $_POST['it_id']) : '';
$it_name = isset($_POST['it_name']) ? trim($_POST['it_name']) : '';
$it_price = isset($_POST['it_price']) ? (int) $_POST['it_price'] : 0;
$it_stock = isset($_POST['it_stock']) ? (int) $_POST['it_stock'] : 0;
...
if(is_checked('chk_ca_it_link'))               $ca_fields .= " , it_link = '$it_link' ";
...
if(is_checked('chk_all_it_link'))               $all_fields .= " , it_link = '$it_link' ";
...
if ($w == "") {
    $sql = "INSERT INTO {$g5['g5_shop_item_table']}
            (it_id, ca_id, it_name, it_price, it_stock, it_date)
            VALUES ('$it_id', '$ca_id', '".sql_real_escape_string($it_name)."', '$it_price', '$it_stock', NOW())";
    sql_query($sql);
    log_history('추가', $it_id);
    alert("상품이 성공적으로 등록되었습니다.", "./itemform.php?w=u&it_id=$it_id");
}

function log_history($action, $target_id) {
    global $g5, $member;
    $sql = "INSERT INTO g5_login_history_save 
            (login_id, user_name, user_ip, accessed_page, history_datetime, action, target_id) 
            VALUES 
            ('{$member['mb_id']}', '{$member['mb_name']}', '{$_SERVER['REMOTE_ADDR']}', '상품 관리', NOW(), '{$action}', '{$target_id}')";
    sql_query($sql);
}
