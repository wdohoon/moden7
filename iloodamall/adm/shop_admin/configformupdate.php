...
function log_history($action_desc) {
    global $g5, $member;
    $sql = "INSERT INTO g5_login_history_save 
            (login_id, user_name, user_ip, accessed_page, history_datetime, action) 
            VALUES 
            ('{$member['mb_id']}', '{$member['mb_name']}', '{$_SERVER['REMOTE_ADDR']}', '쇼핑몰 설정', NOW(), '{$action_desc}')";
    sql_query($sql);
}
...
// acefive
$sql = " update {$g5['g5_shop_default_table']}
            set de_nicepay_mid          = '{$_POST['de_nicepay_mid']}',
                de_nicepay_merchantkey  = '{$_POST['de_nicepay_merchantkey']}',
                de_nicepay_currency     = '{$_POST['de_nicepay_currency']}',
                de_nicepay_goodscl      = '{$_POST['de_nicepay_goodscl']}',
                de_nicepay_cancelpw     = '{$_POST['de_nicepay_cancelpw']}'
                ";
sql_query($sql);

if (sql_query($sql)) {
    log_history("수정"); // 로그 기록
}
// acefive
