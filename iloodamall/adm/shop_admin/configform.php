...
<section id ="anc_scf_payment">
    ...
    <div class="tbl_frm01 tbl_wrap">
		...
		<tr>
            <th scope="row"><label for="cf_use_point">적립금 사용</label></th>
            <td>
                <?php echo help("<a href=\"".G5_ADMIN_URL."/config_form.php#frm_board\" target=\"_blank\">환경설정 &gt; 기본환경설정</a>과 동일한 설정입니다."); ?>
                <input type="checkbox" name="cf_use_money" value="1" id="cf_use_point"<?php echo $config['cf_use_money']?' checked':''; ?>> 사용
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="de_settle_min_money">결제 최소적립금</label></th>
            <td>
                <?php echo help("회원의 적립금이 설정값 이상일 경우만 주문시 결제에 사용할 수 있습니다.\n적립금 사용을 하지 않는 경우에는 의미가 없습니다."); ?>
                <input type="text" name="de_settle_min_money" value="<?php echo get_sanitize_input($default['de_settle_min_money']); ?>" id="de_settle_min_point" class="frm_input" size="10"> 원
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="de_settle_max_point">최대 결제적립금</label></th>
            <td>
                <?php echo help("주문 결제시 최대로 사용할 수 있는 적립금을 설정합니다.\n적립금 사용을 하지 않는 경우에는 의미가 없습니다."); ?>
                <input type="text" name="de_settle_max_money" value="<?php echo get_sanitize_input($default['de_settle_max_money']); ?>" id="de_settle_max_point" class="frm_input" size="10"> 원
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="de_settle_point_unit">결제 적립금단위</label></th>
            <td>
                <?php echo help("주문 결제시 사용되는 적립금의 절사 단위를 설정합니다."); ?>
                <select id="de_settle_point_unit" name="de_settle_money_unit">
                    <option value="100" <?php echo get_selected($default['de_settle_money_unit'], 100); ?>>100</option>
                    <option value="10"  <?php echo get_selected($default['de_settle_money_unit'],  10); ?>>10</option>
                    <option value="1"   <?php echo get_selected($default['de_settle_money_unit'],   1); ?>>1</option>
                </select> 원
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="de_card_point">적립금부여</label></th>
            <td>
                <?php echo help("신용카드, 계좌이체, 휴대폰 결제시 적립금 부여할지를 설정합니다. (기본값은 '아니오')"); ?>
                <select id="de_card_point" name="de_card_money">
                    <option value="0" <?php echo get_selected($default['de_card_money'], 0); ?>>아니오</option>
                    <option value="1" <?php echo get_selected($default['de_card_money'], 1); ?>>예</option>
                </select>
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="de_point_days">주문완료 적립금</label></th>
            <td>
                <?php echo help("주문자가 회원일 경우에만 주문완료시 적립금을 지급합니다. 주문취소, 반품 등을 고려하여 포인트를 지급할 적당한 기간을 입력하십시오. (기본값은 7일)\n0일로 설정하는 경우에는 주문완료와 동시에 적립금 지급합니다."); ?>
                주문 완료 <input type="text" name="de_money_days" value="<?php echo get_sanitize_input($default['de_money_days']); ?>" id="de_point_days" class="frm_input" size="2"> 일 이후에 포인트를 지급
            </td>
        </tr>
            ...
            <td>
                ...
                <ul class="de_pg_tab">
                    ...
                    <li class="<?php if($default['de_pg_service'] == 'inicis') echo 'tab-current'; ?>"><a href="#inicis_info_anchor" data-value="inicis" title="KG이니시스 선택하기">KG이니시스</a></li>
					 <!-- acefive -->
					<li class="<?php if($default['de_pg_service'] == 'nicepay') echo 'tab-current'; ?>"><a href="#nicepay_info_anchor" data-value="nicepay" title="나이스페이 선택하기">나이스페이</a></li> <!-- acefive -->
                </ul>
            </td>
        </tr>
		<!-- acefive -->
        ...
        if(!isset($default['de_nicepay_mid'])) {
            sql_query(" ALTER TABLE `{$g5['g5_shop_default_table']}`
                            ADD  `de_nicepay_mid` VARCHAR( 255 ) NOT NULL ,
                            ADD  `de_nicepay_merchantkey` VARCHAR( 255 ) NOT NULL ,
                            ADD  `de_nicepay_currency` VARCHAR( 3 ) NOT NULL ,
                            ADD  `de_nicepay_goodscl` tinyint( 4 ) NOT NULL,
                            ADD  `de_nicepay_cancelpw` VARCHAR( 30 ) NOT NULL
                            ; ", true);
        }

        $log_path = G5_SHOP_PATH.'/nicepay/log';

        if(!is_dir($log_path)) {
            echo '<script>'.PHP_EOL;
            echo 'alert("'.str_replace(G5_PATH.'/', '', G5_SHOP_PATH).'/nicepay 폴더 안에 log 폴더를 생성하신 후 쓰기권한을 부여해 주십시오.\n> mkdir log\n> chmod 707 log");'.PHP_EOL;
            echo '</script>'.PHP_EOL;
        } else {
        ...
        if (!class_exists('DOMDocument')) {
            echo '<script>'.PHP_EOL;
            echo 'alert("Class \'DOMDocument\' not found.\n서버 관리자에게 문의해 주십시오.");'.PHP_EOL;
            echo '</script>'.PHP_EOL;
        }
        if (!function_exists('socket_create')) {
            echo '<script>'.PHP_EOL;
            echo 'alert("SOCKET 관련 함수를 사용할 수 없습니다.\n정상적으로 결제가 진행되지 않을경우 서버 관리자에게 문의해 주십시오.");'.PHP_EOL;
            echo '</script>'.PHP_EOL;
        }
        ?>
        <tr class="pg_info_fld nicepay_info_fld">
            <th scope="row">나이스페이 상점아이디(MID)</th>
            <td><input type="text" name="de_nicepay_mid" value='<?php echo $default['de_nicepay_mid']?>' class="frm_input" size="20"></td>
        </tr>
        <tr class="pg_info_fld nicepay_info_fld">
            <th scope="row">나이스페이 상점서명키</th>
            <td>
                <?php echo help("NICEPAY 가맹점관리자 페이지 : https://pg.nicepay.co.kr/home/Login.jsp"); ?>
                <?php echo help("가맹점 정보 – 가맹점정보 – KEY 관리에서 MID와 상점키를 확인합니다."); ?>
                <input type="text" name="de_nicepay_merchantkey" value='<?php echo $default['de_nicepay_merchantkey']?>' class="frm_input" size="20">
            </td>
        </tr>
        <tr class="pg_info_fld nicepay_info_fld">
            <th scope="row">나이스페이 취소비밀번호</th>
            <td>
                <?php echo help("취소 비밀번호는 가맹점 관리자의 가맹점정보 &gt; 기본 정보 &gt; 에서 별도로 설정한 비밀 번호를 기입합니다."); ?>
                <input type="password" name="de_nicepay_cancelpw" value='<?php echo $default['de_nicepay_cancelpw']?>' class="frm_input" size="20">
            </td>
        </tr>
        <tr class="pg_info_fld nicepay_info_fld">
            <th scope="row">나이스페이 휴대폰 결제 구분</th>
            <td>
                <?php echo help("휴대폰 결제시 사용될 상품구분을 선택해주세요."); ?>
                <input type="radio" name="de_nicepay_goodscl" value="0" <?php echo $default['de_nicepay_goodscl']==0?"checked":""; ?> id="de_nicepay_goodscl1">
                <label for="de_nicepay_goodscl1">컨덴츠</label>
                <input type="radio" name="de_nicepay_goodscl" value="1" <?php echo $default['de_nicepay_goodscl']==1?"checked":""; ?> id="de_nicepay_goodscl2">
                <label for="de_nicepay_goodscl2"> 실물</label>
            </td>
        </tr>
        <!-- acefive -->
