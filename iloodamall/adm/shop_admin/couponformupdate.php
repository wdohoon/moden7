...
// 로그 저장 함수
function log_coupon_action($action, $cp_id, $details) {
    global $g5, $member;
    $sql = "INSERT INTO g5_login_history_save
                (login_id, user_name, user_ip, accessed_page, history_datetime, action, target_id)
            VALUES
                ('{$member['mb_id']}', '{$member['mb_name']}', '{$_SERVER['REMOTE_ADDR']}', '쿠폰 관리', NOW(), '$action', '$cp_id')";
    sql_query($sql);
}

if ($w == '') {
    sql_query($sql);
    log_coupon_action('추가', $cp_id, $mb_id, "쿠폰 생성");
} else if ($w == 'u') {
    sql_query($sql);
    log_coupon_action('수정', $cp_id, $mb_id, "쿠폰 수정");
} else if ($w == 'd') {
    sql_query($sql);
    log_coupon_action('삭제', $cp_id, $mb_id, "쿠폰 삭제");
}
