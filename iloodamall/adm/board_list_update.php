...
if ($act_button === "선택수정") {
    for ($i = 0; $i < $post_count_chk; $i++) {
        $k = (int) $_POST['chk'][$i];
        $bo_table = clean_xss_tags($board_table[$k], 1, 1);
        $p_bo_subject = isset($_POST['bo_subject'][$k]) ? clean_xss_tags($_POST['bo_subject'][$k], 1, 1) : '';

        $sql = "UPDATE {$g5['board_table']} SET bo_subject = '$p_bo_subject' WHERE bo_table = '$bo_table'";
        $result = sql_query($sql);
        if ($result) {
            record_action_log($member['mb_id'], $member['mb_name'], '수정', $bo_table);
        }
    }
} elseif ($act_button === "선택삭제") {
    ...
    for ($i = 0; $i < $post_count_chk; $i++) {
        ...
        $result = sql_query($sql);
        if ($result) {
            record_action_log($member['mb_id'], $member['mb_name'], '삭제', $bo_table);
        }
    }
}

function record_action_log($login_id, $user_name, $action, $target_id) {
    global $g5;
