...
<?php
$sql_common = " from {$g5['point2_table']} po";
...
$sql = " select po.*, mb.mb_name, mb.mb_nick, mb.mb_email, mb.mb_homepage, mb.mb_point2
            {$sql_common}
...
if ($config['cf_money_term'] > 0) {
    $po_expire_term = $config['cf_money_term'];
}
...
// 여기서 새로운 쿼리를 정의해야 합니다.
$sql = " select po.*, mb.mb_name, mb.mb_nick, mb.mb_email, mb.mb_homepage, mb.mb_point2,mb_1
            {$sql_common}
            LEFT JOIN {$g5['member_table']} mb ON po.mb_id = mb.mb_id 
            {$sql_search}
            {$sql_order}
...
<div class="local_ov01 local_ov">
    ...
    if (isset($mb['mb_id']) && $mb['mb_id']) {
        echo '&nbsp;<span class="btn_ov01"><span class="ov_txt">' . $mb['mb_id'] . ' 님 적립금 합계 </span><span class="ov_num"> ' . number_format($mb['mb_point']) . '점</span></span>';
    } else {
        $totalSum = sql_fetch(" select sum(po_point) as sum_point from {$g5['point2_table']} ");
        $totalUsed = sql_fetch(" select sum(po_point) as sum_point from {$g5['point2_table']} where po_point < 0");
        $totalEarned = $totalSum['sum_point'] + $totalUsed['sum_point']; // 전체 지급량 계산

        echo '&nbsp;<a href="' . $_SERVER['SCRIPT_NAME'] . '?mode=earned" class="btn_ov01"><span class="btn_ov01"><span class="ov_txt">전체 합계</span><span class="ov_num">' . number_format($totalEarned) . '점 </span> </span></a>';

        echo '&nbsp;<a href="' . $_SERVER['SCRIPT_NAME'] . '" class="btn_ov01"><span class="btn_ov01"><span class="ov_txt">전체 지급량</span><span class="ov_num">' . number_format($totalSum['sum_point']) . '점 </span></span></a>';

        echo '&nbsp;<a href="' . $_SERVER['SCRIPT_NAME'] . '?mode=used" class="btn_ov01"><span class="btn_ov01"><span class="ov_txt">전체 사용량</span><span class="ov_num">' . number_format($totalUsed['sum_point']) . '점 </span> </span></a>';
    }
    ?>
</div>
...
<form name="fpointlist" id="fpointlist" method="post" action="./point2_list_delete.php" onsubmit="return fpointlist_submit(this);">
        ...
        <table>
            ...
            <thead>
                <tr>
                    ...
                    <th scope="col">이름</th>
                    <th scope="col">병원명</th>
                    <th scope="col"><?php echo subject_sort_link('po_content') ?>적립금 내용</a></th>
                    <th scope="col"><?php echo subject_sort_link('po_point') ?>적립금</a></th>
    ...
    <div class="btn_fixed_top">
		<a href="/adm/excel_mp.php" class="btn btn_01">회원별 MP 현황</a>
...
// 기존 코드...
$sql3 = " select count(*) as cnt {$sql_common} {$sql_search} {$sql_order} ";
$row3 = sql_fetch($sql3);
...
switch ($mode) {
    case 'earned':
        $sql_count = "SELECT COUNT(*) AS cnt FROM {$g5['point2_table']} WHERE po_point > 0";
        break;
    case 'used':
        $sql_count = "SELECT COUNT(*) AS cnt FROM {$g5['point2_table']} WHERE po_point < 0";
        break;
    default:
        $sql_count = $sql; // 기본 모드일 경우, 기존 쿼리 사용
        break;
}
...
<section id="point_mng">
    <h2 class="h2_frm">개별회원 적립금 증감 설정</h2>

    <form name="fpointlist2" method="post" id="fpointlist2" action="./point2_update.php" autocomplete="off">
        <input type="hidden" name="sfl" value="<?php echo $sfl ?>">
        <input type="hidden" name="stx" value="<?php echo $stx ?>">
        <input type="hidden" name="sst" value="<?php echo $sst ?>">
        <input type="hidden" name="sod" value="<?php echo $sod ?>">
        ...
        <div class="tbl_frm01 tbl_wrap">
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        <th scope="row"><label for="po_content">적립금 내용<strong class="sound_only">필수</strong></label></th>
                        <td><input type="text" name="po_content" id="po_content" required class="required frm_input" size="80"></td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="po_point">적립금<strong class="sound_only">필수</strong></label></th>
                        <td><input type="text" name="po_point" id="po_point" required class="required frm_input"></td>
                    </tr>
                    <?php if ($config['cf_money_term'] > 0) { ?>
                        <tr>
                            <th scope="row"><label for="po_expire_term">적립금 유효기간</label></th>
