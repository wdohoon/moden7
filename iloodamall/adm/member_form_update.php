...
$mb_recommend = isset($_POST['mb_recommend']) ? trim(strip_tags($_POST['mb_recommend'])) : '';
...
if ($w == '' || $w == 'u') {
    ...
    if ($config['cf_use_recommend'] && $mb_recommend){
        ...
        //insert_point2($mb_id, $config['cf_recommend_money'], $mb_recommend.'를 추천인등록', '@member', $mb_recommend, $mb_id.' 추천');
	}
...
// 회원 정보가 성공적으로 추가된 후에 실행할 코드
if ($w == '') {
    // 회원 정보가 데이터베이스에 추가된 후 로그인 이력을 저장
    $sql = "
    INSERT INTO g5_login_history_save (
        login_id, 
        history_datetime, 
        user_name, 
        accessed_page, 
...
if ($w == 'u') {
    // 데이터베이스 업데이트 로직
    $sql_update = "UPDATE {$g5['member_table']} SET {$sql_common} WHERE mb_id = '$mb_id'";
    ...
    if ($result_update) {
        ...
        $sql_log = "INSERT INTO g5_login_history_save (
                        ...
                        '".sql_real_escape_string($member['mb_name'])."', 
        ...
        if (!$result_log) {
            // 로그 기록 실패 시 오류 메시지 로깅
            error_log('Failed to record log in g5_login_history_save: ' . mysqli_error($GLOBALS['link']));
        }
    } else {
        // 업데이트 실패 시 오류 메시지
        error_log('Failed to update member info: ' . mysqli_error($GLOBALS['link']));
    }
}
