...
if ($_POST['act_button'] == "선택수정") {
    for ($i = 0; $i < count($_POST['chk']); $i++) {
        $k = (int) $_POST['chk'][$i];  // 실제 번호를 넘김
        $mb = get_member($_POST['mb_id'][$k]); // 해당 회원 정보 가져오기

        // 선택된 회원에 대한 정보 업데이트와 로그 기록
        if ($mb['mb_id']) {
            $sql = " update {$g5['member_table']}
                        set mb_level = '" . $post_mb_level . "',
                            mb_intercept_date = '" . sql_real_escape_string($post_mb_intercept_date) . "',
            ...
            // 로그 기록
            $sql_log = "INSERT INTO g5_login_history_save (login_id, user_name, user_ip, accessed_page, history_datetime, action, target_id) 
                        VALUES ('{$member['mb_id']}', '{$member['mb_name']}', '{$_SERVER['REMOTE_ADDR']}', '회원관리', NOW(), '수정', '{$mb['mb_id']}')";
            sql_query($sql_log);
        }
    }
}
