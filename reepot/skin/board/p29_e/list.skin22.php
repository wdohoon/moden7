...
<style>
#stroe_wrap {overflow:hidden;width:100%; min-height:274px; background:#f9f9f9; position:relative;margin-bottom:50px;padding:30px 9%;}
...
.map_wrap {position:relative;float:left;width:50%;height:386px;}
.stoer_map {display:block;position:absolute; left:50%; top:0px; margin-left:-104px;background:url("/skin/board/<?=$board['bo_skin']?>/map00.jpg") no-repeat;}
.stoer_map img {position:relative; z-index:33333;}
.store_info {display:block;position:absolute;width:288px;height:386px; left:50%; top:0px; margin-left:-104px;}
...
.jtbl_wrap table thead{display:none}
...
.bo_fx{position: absolute;top: 170px;right:0;z-index: 1;margin-right: 14.3229vw;}
.jbutton.black a{background-image: none;color: #fff!important;font-size: 0.8854vw;border: 1px solid #fff;border-radius: 5px;height: 40px;line-height: 40px;width: 120px;}
...
<div class="top_wrap">
  <div class="inner_bg">
    ...
    <div class="text-top">
      <h3>병원찾기</h3>
      <p>당신과 함께 할 가까운 리팟 케렌시아를 찾아보세요</p>
    </div>
 ...
 <?php
      ...
      <div class="bo_fx">
        ...
        <ul class="btn_bo_user">
          <?php
          if ($admin_href) {
          ?><li><span class="jbutton large black"><a href="<?php echo $admin_href ?>">관리자</a></span></li><?php
          }
          ?>
          <?php
          if ($list_href) {
          ...
          <?php
          if ($write_href) {
          ?><li><span class="jbutton large black"><a href="<?php echo $write_href ?>" class="width_j">병원등록</a></span></li><?php
          }
          ?>
        </ul>
        <?php
      ...
      <div class="jtbl_head01 jtbl_wrap">
        ...
        <?php
        if (count($list) == 0) {
          echo '<tr><td colspan="8" class="empty_table">게시물이 없습니다.</td></tr>';
        }
        ?>
        </tbody>
        </table>
