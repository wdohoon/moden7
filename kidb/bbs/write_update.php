...
if($bo_table == 'inquiry'){
	$privacy = 'on' ? trim($_POST['privacy_policy_agreement']) : '';
	if($privacy != 'on'){
		alert('개인정보취급방침에 동의해주세요.', '/bbs/write.php?bo_table=inquiry');
	}

	$_POST['wr_subject'] = $_POST['wr_name'].'님의 문의사항';
}
...
if ($bo_table == 'leadership') {
    if ($wr_subject =='') {
			$_POST['wr_subject'] = $_POST['wr_8'];
        //exit;
    }
    
    if ($wr_content == '') {
		$_POST['wr_content'] = $_POST['wr_8'];
        //exit;
    }
}
