...
function get_paging2($write_pages, $cur_page, $total_page, $url, $add="")
{
    ...
    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
			$nbsp = "";
			if($k==$start_page && $cur_page > 1) $nbsp = " nbsp1";
			if($k==$end_page && $cur_page < $total_page) $nbsp = " nbsp2";

            if ($cur_page != $k)
                $str .= '<a href="'.$url.$k.$add.'" class="pg_page'.$nbsp.'">'.$k.'<span class="sound_only">페이지</span></a>'.PHP_EOL;
            else
                $str .= '<span class="sound_only">열린</span><strong class="pg_current'.$nbsp.'">'.$k.'</strong><span class="sound_only">페이지</span>'.PHP_EOL;
        }
    }
...
<div id="bo_list" style="width:<?php echo $width; ?>">
    ...
    <div class="tbl_head03 tbl_wrap">
        ...
        <tr>
            ...
            <th scope="col">NO</th>
            <th scope="col">제목</th>
            <th scope="col" class="pcc">글쓴이</th>
			<th scope="col" class="pcc">작성일</th>
            <th scope="col" class="pcc">조회수</th>        
        </tr>
        </thead>
        <tbody>
        <?php
            ...
            <td class="td_subject" style="padding-left:<?php echo $list[$i]['reply'] ? (strlen($list[$i]['wr_reply'])*10) : '5'; ?>px">
                ...
                <div class="bo_tit">
					...
					<div class="mo_info mmm">
						<span><i class="fa fa-user"></i> <?php echo $list[$i]['name'] ?> </span> &nbsp;
						<span><i class="fa fa-clock-o" aria-hidden="true"></i> <?php echo $list[$i]['datetime2'] ?></span> &nbsp;
						<span><i class="fa fa-eye" aria-hidden="true"></i> <?php echo $list[$i]['wr_hit'] ?></span>
					</div>
                </div>

            </td>
            <td class="td_name sv_use pcc"><?php echo $list[$i]['name'] ?></td>
			<td class="td_datetime pcc"><?php echo $list[$i]['datetime'] ?></td>
            <td class="td_num pcc"><?php echo $list[$i]['wr_hit'] ?></td>
        </tr>
        <?php } ?>
