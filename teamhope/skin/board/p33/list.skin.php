...
<style>
	...
	#bo_list thead th { height: 40px; font-size: 18px; color: #121212; font-weight: 400; background-color: #f7f7f7; border: 1px solid #e1e1e1; text-align: center; }
...
<table width="1280px;" border="0" cellspacing="0" cellpadding="0" style="margin: 0 auto;">
<thead>
  <tr>
...
<?php
		...
		if ($row['wr_id'] != $sc_id && date('Ymd', strtotime($row['wr_1'])) <= date('Ymd') && date('Ymd', strtotime($row['wr_2'])) >= date('Ymd')) {
			...
			$today_schedule .= " <b>".$row['wr_subject']."</b>";
...
<div class="today_schedule">
    ...
    <ul id="daily_schedule">
        <?php
        if (!empty($today_schedule)) {
            echo "<li>$today_schedule</li>"; 
        } else {
            echo "<li><span style='cursor:auto;'>오늘의 일정이 없습니다.</span></li>";
        }
        ?>
    </ul>
</div>
