...
$col_height= 140 ;//내용 들어갈 사각공간의 세로길이를 가로 폭과 같도록
$today = getdate(); 
$b_mon = $today['mon']; 
$b_day = $today['mday']; 
$b_year = $today['year']; 
...
<div style="height: 1px;background: #131f36;margin-bottom: 30px;"></div>


<?php

$today2 = date('Ymd', G5_SERVER_TIME);

$sql_where = " where wr_is_comment = 0 and wr_1 <= $today2 and wr_2 >= $today2 ";
$sql = "select wr_id, wr_subject from {$write_table} {$sql_where} ";
$result = sql_query($sql);
$total = sql_num_rows($result);
?>

<div class="today_schedule">
	<span class="today"><?php echo date('Y년 m월 d일', G5_SERVER_TIME) ?></span>
	<ul>
		<?php
		for($i=0;$row=sql_fetch_array($result);$i++) {
			$bo_href = '/bbs/board.php?bo_table='.$bo_table.'&wr_id='.$row['wr_id'];
		?>
		<li><a href="<?php echo $bo_href ?>"><?php echo $row['wr_subject'] ?></a></li>
		<?php
		}
		if($total == 0) { echo "<li><span style='cursor:auto;'>오늘 일정이 없습니다.</span></li>"; }
		?>
...
<table width="<?php echo $width ?>" border="0" cellpadding="0" cellspacing="0">
  <tr>
       ...
       <td width="60%" height="40" align="center">
		...
		<tr>
			<td><a href="<?php echo $_SERVER[PHP_SELF]."?bo_table=".$bo_table."&"; ?><?php if ($month == 1) { $year_pre=$year-1; $month_pre=$month; } else {$year_pre=$year-1; $month_pre=$month;} echo ("year=$year_pre&month=$month_pre&sc_no=$sc_no");?>"><img src="<?php echo $board_skin_url?>/img/y_prev2.png" border="0" alt="<?php echo $year_pre?>년" style="margin-right:3px"></a></td>
			<td><a href="<?php echo $_SERVER[PHP_SELF]."?bo_table=".$bo_table."&"; ?><?php if ($month == 1) { $year_pre=$year-1; $month_pre=12; } else {$year_pre=$year; $month_pre=$month-1;} echo ("year=$year_pre&month=$month_pre&sc_no=$sc_no");?>"><img src="<?php echo $board_skin_url?>/img/m_prev2.png" border="0" alt="<?php echo $month_pre?>월"></a></td>
			<td style="padding:0 30px;font-size:21px;font-weight:500;">
				<a href="<?php echo $_SERVER[PHP_SELF]."?bo_table=".$bo_table; ?>" title="오늘로" onfocus="this.blur()" style="color:#121212;"><?php echo ("$year"."년"."&nbsp;$month"."월"); ?></a>
			</td>
			<td><a href="<?php echo $_SERVER[PHP_SELF]."?bo_table=".$bo_table."&"; ?><?php if ($month == 12) { $year_pre=$year+1; $month_pre=1; } else {$year_pre=$year; $month_pre=$month+1;} echo ("year=$year_pre&month=$month_pre&sc_no=$sc_no");?>"><img src="<?php echo $board_skin_url?>/img/m_next2.png" border="0" alt="<?php echo $month_pre?>월"></a></td>
			<td><a href="<?php echo $_SERVER[PHP_SELF]."?bo_table=".$bo_table."&"; ?><?php if ($month == 12) { $year_pre=$year+1; $month_pre=$month; } else {$year_pre=$year+1; $month_pre=$month;} echo ("year=$year_pre&month=$month_pre&sc_no=$sc_no");?>"><img src="<?php echo $board_skin_url?>/img/y_next2.png" border="0" alt="<?php echo $year_pre?>년" style="margin-left:3px"></a></td>
		</tr>
		</table>			
	</td>
...
#bo_list thead th { height: 40px; font-size: 14px; color: #121212; font-weight: 400; background-color: #f7f7f7; border: 1px solid #e1e1e1; text-align: center; }
#bo_list tbody td { font-size: 13px; color: #121212; font-weight: 400; letter-spacing: -0.02em; padding: 8px; border: 1px solid #e1e1e1; }
#bo_list .bo_tit { display: block; font-size: 12px; color: #1f1f1f; font-weight: 400; line-height: 18px;  }
</style>
...
<thead>
  <tr>
	<th style="color:#e30000">일요일</th>
	<th>월요일</th>
	<th>화요일</th>
	<th>수요일</th>
	<th>목요일</th>
	<th>금요일</th>
	<th style="color:#0060e3">토요일</th>
  </tr>
</thead>
<tbody>
<?php
...
while ($row = sql_fetch_array($result)) {  // 제목글 뽑아서 링크 문자열 만들기..
  ...
  for ($i = $start_day ; $i <= $end_day;  $i++) {
	...
	if ($member[mb_level] < $board[bo_read_level]) {
		$html_day[$i].= "<span id='subject_".$j."' class='bo_tit'>-".cut_str($row['wr_subject'],12,'...')."</span>";
	} else {
...
<?php
  ...
  for ($iz = 1; $iz <= $lastcount; $iz++) { // 42번을 칠하게 된다.
    ...
    if ($write_href) { 
      ...
      echo "<a href='$write_href&f_date=$f_date' style='display:inline-block;margin-bottom:5px;'><font color='$daycolor' title='일정추가'>$daytext</font></a>$moonday <font color='$daycontcolor'>$daycont</font>$blank $annivmoonday";
    } else { // 글쓰기 권한이 없으면 글쓰기 링크는 넣지 않고 그냥 숫자와 기념일 내용만 출력하기  
      echo "<font color='$daycolor' style='display:inline-block;margin-bottom:5px;'>$daytext</font>$moonday <font color='$daycontcolor'>$daycont</font>$blank $annivmoonday";
    }
    echo $html_day[$cday];
    echo ("</td>");  // 한칸을 마무리
    $cday++; // 날짜를 카운팅
