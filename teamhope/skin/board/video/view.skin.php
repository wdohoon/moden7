...
<article id="bo_v">
    ...
    <section id="bo_v_atc">
		...
		<p id="v_tt"><?php echo cut_str(get_text($view['wr_subject']), 100); // 글제목 출력 ?></p>
		...
		<?
		if($view['wr_2']){
			$v_width = $view['wr_2'];   // 동영상 넓이 지정
		}else{
			$v_width = '800';   // 동영상 넓이 지정
		}
		?>

		<!-- youtube -->
		<?php if($view['wr_1']){ ?>
		<style>#bo_v_img { display: none; }</style>
		<div style="position:relative;width: 100%;max-width:<?php echo $v_width?>px;margin:0px auto 20px;">
			<div class="embed-responsive embed-responsive-16by9"> 
				<iframe  class="embed-responsive-item" src="http://www.youtube.com/embed/<?php echo $view[wr_1]?>?feature=player_embedded&rel=0" allowfullscreen></iframe>
			</div>
		</div>
		<?php } ?>
		...
		<div style="position:relative;width: 100%;max-width:<?php echo $v_width?>px;margin:0px auto 20px;">
			<div class="embed-responsive embed-responsive-16by9"> 
				<video id="video_upload" class="video-js vjs-default-skin embed-responsive-item" controls preload="none" width="100%" height="100%" poster="<?php echo $v_logo?>" data-setup="{}">
		...
		document.getElementById("video_upload").volume = 0.5;
		</script>
		<?php } ?>		
		...
		<div style="margin:20px auto;">
			<audio id="myaudio" controls="controls">
				<source src="<?=G5_URL."/data/file/".$bo_table."/".$view[file][2][file]?>" />
			</audio>
		</div>
		<script>
    ...
    if ($view['file']['count']) {
        for ($i=0; $i<count($view['file']); $i++) {
            if (isset($view['file'][$i]['source']) && $view['file'][$i]['source'] && !$view['file'][$i]['view'] && !preg_match("/\.(mp3|mp4|ogg|wav)$/i", $view['file'][$i]['file']))
                $cnt++;
        }
    }
     ?>
    ...
    <div id="bo_v_top">
        ...
        <ul class="bo_v_nb">
            <?php if ($prev_href) { ?>
			<li class="btn_prv" onclick="location.href='<?php echo $prev_href ?>'">
				<span class="nb_tit">Previous Creative</span>
				<p><?php echo $prev_wr_subject?></p>
			</li>
			<?php } else { ?>
			<li class="btn_prv" style="cursor: auto;">
				<span class="nb_tit">Previous Creative</span>
				<p>이전글이 없습니다.</p>
			</li>
			<?php } ?>
            <?php if ($next_href) { ?>
			<li class="btn_next" onclick="location.href='<?php echo $next_href ?>'">
				<span class="nb_tit">Next Creative</span>
				<p><?php echo $next_wr_subject?></p>
			</li>
			<?php } else { ?>
			<li class="btn_next" style="cursor: auto;">
				<span class="nb_tit">Next Creative</span>
				<p>다음글이 없습니다.</p>
			</li>
			<?php } ?>
